function plot_forw_k(start_ang_l, start_ang_r, end_ang_l, end_ang_r)
    ...
    start_ang_l = wrapTo360(start_ang_l);
    end_ang_l = wrapTo360(end_ang_l);
    start_ang_r = wrapTo180(start_ang_r);
    end_ang_r = wrapTo180(end_ang_r);

    for i = 2:n
       cur_ang_l = start_ang_l * (n - i) / n + end_ang_l * i / n;
       cur_ang_r = start_ang_r * (n - i) / n + end_ang_r * i / n;
       [xt,yt,x2,y2,x3,y3,x4,y4,x5,y5] = forw_k_4plot(cur_ang_l, cur_ang_r);
