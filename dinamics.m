...
ML_alphaL = extract_term(S.ML, alphaL);
ML_alpha3 = extract_term(S.ML, alpha3);
ML_alpha4 = extract_term(S.ML, alpha4);
ML_alphaR = extract_term(S.ML, alphaR);
ML_omegaL_sqr = extract_term(S.ML, omegaL);
ML_omega3_sqr = extract_term(S.ML, omega3);                                 % nema omega4 ovde
ML_omegaR_sqr = extract_term(S.ML, omegaR);

MR_alphaL = extract_term(S.MR, alphaL);
MR_alpha3 = extract_term(S.MR, alpha3);
MR_alpha4 = extract_term(S.MR, alpha4);
MR_alphaR = extract_term(S.MR, alphaR);
MR_omegaL_sqr = extract_term(S.MR, omegaL);                                 % nema omega3 ovde
MR_omega4_sqr = extract_term(S.MR, omega4);
MR_omegaR_sqr = extract_term(S.MR, omegaR);

ML_simple = ML_alphaL*alphaL + ML_alpha3*alpha3 + ML_alpha4*alpha4 + ML_alphaR*alphaR + ML_omegaL_sqr*omegaL^2 + ML_omega3_sqr*omega3^2 + ML_omegaR_sqr*omegaR^2;
MR_simple = MR_alphaL*alphaL + MR_alpha3*alpha3 + MR_alpha4*alpha4 + MR_alphaR*alphaR + MR_omegaL_sqr*omegaL^2 + MR_omega4_sqr*omega4^2 + MR_omegaR_sqr*omegaR^2;