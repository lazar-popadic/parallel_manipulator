function [left_ang_deg, right_ang_deg] = inv_k(xt,yt)
    ...
    sol_theta5(1) = 2*atan2(1+sqrt(1-C3*C3+C4*C4), C3+C4);
    sol_theta5(2) = 2*atan2(1-sqrt(1-C3*C3+C4*C4), C3+C4);
    sol_theta5(1) = wrapToPi(sol_theta5(1));
    sol_theta5(2) = wrapToPi(sol_theta5(2));

     % Take the right solution for +- configuration
     % TODO: izracunaj i theta3 i 4 zbog ovoga
     

    left_ang_deg = rad2deg(double(sol_theta2(1)));
    right_ang_deg = rad2deg(double(sol_theta5(2)));
end